import subprocess, json, datetime, collections, os

GH = '/opt/homebrew/bin/gh'
ENV = {**os.environ, 'PATH': '/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin'}

now = datetime.datetime.now(datetime.timezone.utc)
since_dt = now - datetime.timedelta(days=60)
prev_dt = since_dt - datetime.timedelta(days=60)

# Get login
try:
    login = subprocess.run([GH, 'api', 'user', '-q', '.login'], capture_output=True, text=True, timeout=15, env=ENV).stdout.strip()
except Exception:
    login = ''

# Fetch repos sorted by most recently pushed (most likely to have recent runs)
try:
    r = subprocess.run(
        [GH, 'api', 'user/repos', '--paginate',
         '-q', f'.[] | select(.owner.login == "{login}") | [.pushed_at, .full_name] | @tsv'],
        capture_output=True, text=True, timeout=30, env=ENV
    )
    lines = sorted([l for l in r.stdout.strip().split('\n') if '\t' in l], reverse=True)
    repos = [l.split('\t')[1] for l in lines][:20]
except Exception:
    repos = []

day_counts = collections.defaultdict(int)
total = success = ms = ptotal = psuccess = pms = 0

for repo in repos:
    try:
        page = 1
        while True:
            r = subprocess.run(
                [GH, 'api', f'repos/{repo}/actions/runs?per_page=100&page={page}'],
                capture_output=True, text=True, timeout=20, env=ENV
            )
            if r.returncode != 0 or not r.stdout.strip():
                break
            data = json.loads(r.stdout)
            runs = data.get('workflow_runs', [])
            if not runs:
                break
            any_in_range = False
            for run in runs:
                dt_str = run.get('run_started_at') or run.get('created_at', '')
                if not dt_str:
                    continue
                try:
                    run_dt = datetime.datetime.fromisoformat(dt_str.replace('Z', '+00:00'))
                except Exception:
                    continue

                # Duration: prefer run_duration_ms, fall back to timestamp diff
                duration = run.get('run_duration_ms', 0) or 0
                if not duration:
                    try:
                        t0 = datetime.datetime.fromisoformat((run.get('run_started_at') or run.get('created_at', '')).replace('Z', '+00:00'))
                        t1 = datetime.datetime.fromisoformat(run.get('updated_at', '').replace('Z', '+00:00'))
                        duration = max(0, int((t1 - t0).total_seconds() * 1000))
                    except Exception:
                        pass

                conclusion = run.get('conclusion')
                is_success = conclusion == 'success'

                if run_dt >= since_dt:
                    any_in_range = True
                    day_counts[run_dt.strftime('%Y-%m-%d')] += 1
                    total += 1
                    if is_success:
                        success += 1
                    ms += duration
                elif run_dt >= prev_dt:
                    any_in_range = True
                    ptotal += 1
                    if is_success:
                        psuccess += 1
                    pms += duration

            if len(runs) < 100 or not any_in_range:
                break
            page += 1
    except Exception:
        continue

max_count = max(day_counts.values()) if day_counts else 1

print(json.dumps({
    'days': dict(day_counts),
    'maxCount': max_count,
    'totalRuns': total,
    'successRate': round(success / total * 100, 1) if total else 0,
    'totalMinutes': ms // 60000,
    'prevTotalRuns': ptotal,
    'prevSuccessRate': round(psuccess / ptotal * 100, 1) if ptotal else 0,
    'prevTotalMinutes': pms // 60000,
}))
